...
- `prompt.ts` — last-user-prompt extraction (`extractUserPrompt`/`extractUserPromptBlocks`), `TOOL_NAMING_CLARIFICATION`, `toPromptArray` (normalizes the `string`-vs-`string[]` system-prompt type mismatch), system-prompt-append assembly.
