...
export function formatToolAction(tc: ToolCallState): string | undefined {
  ...
  if (verb === "read" || verb === "readfile") {
    ...
    const pat =
      typeof input?.pattern === "string" ? input.pattern.slice(0, 40) : "";
    ...
    const input = tc.rawInput as Record<string, unknown> | undefined;
    const name =
      typeof input?.skill === "string" ? input.skill.slice(0, 40) : "";
    ...
    const todos = Array.isArray((tc.rawInput as any)?.todos)
      ? (tc.rawInput as any).todos
      : [];
    const current =
